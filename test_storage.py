...
def test_local_storage():
    ...
    assertEqual(lss.get(key)['data'], 'test content version 2')
    ...
    assertEqual(lss.get(key)['data'], 'test content version 1')
    print("The reverted content is as expected")

    lss.revert(key, versions[0]['version'])
    assertEqual(lss.get(key)['data'], 'test content version 2')
    assertEqual(lss.get(key, versions[1]['version'])['data'], 'test content version 2')
    print("It support revert revert")

    lss.revert(key, versions[-1]['version'])
    assertEqual(lss.get(key)['data'], '')
